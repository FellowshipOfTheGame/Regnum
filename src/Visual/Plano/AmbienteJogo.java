...
public class AmbienteJogo extends Ambiente {
    ...
    /**
     * Vetor que guarda informações da ultimo campo clicado
     * campoFLC[0] guarda a face do campo
     * campoFLC[1] guarda a linha do campo
     * campoFLC[2] guarda a coluna do campo
     */
    ...
    /**
     * Detecta qual campo foi clicado, dado a posicao do mouse, e retorna a face,
     * linha e coluna.
     *
     * @param x Posicao X do mouse em relacao ao centro
     * @param y Posicao Y do mouse em relacao ao centro
     * @return int[3] com os valores da face, linha e coluna nas posicoes 0, 1 e 2.
     */
    ...
    public void desenha(Graphics2D g2d) {
        ...
        if(this.campoFLC != null){
            ...
            g2d.fillRect(0, 0, this.larguraAnterior/3, this.alturaAnterior);
            ...
            int size = (j2 != 0)? 2 : 1;
            int h = this.alturaAnterior/size - 30*(size-1);
            h = (h < this.larguraAnterior/3)? h : this.larguraAnterior/3;
            
            Image peca = this.mapaImagens.getImageCampo(p1, j1, v1, janela);
            g2d.drawImage(peca, 0, 0, h, h, janela);
            for(int i=0; i<v1; i++){
                g2d.setColor(Color.red);
                g2d.fillRect(h/2-(v1*15)/2+15*i, 10, 10, 10);
            }
            if(size == 2){
                int p2 = (id % 100) / 10;
                int v2 = (id % 10);
                ...
                g2d.drawImage(peca, 0, h+30, h, h, janela);
                for(int i=0; i<v2; i++){
                    g2d.setColor(Color.red);
                    g2d.fillRect(h/2-(v2*15)/2+15*i, h+30+10, 10, 10);
                }
            }
